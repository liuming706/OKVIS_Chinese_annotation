...
TEST(OkvisVioInterfaces, testFrameSync)
{
    ...
    ON_CALL(dummy, multiFrame(_))
        .WillByDefault(Return(std::shared_ptr<okvis::MultiFrame>(
            new okvis::MultiFrame(parameters.nCameraSystem, okvis::Time::now()))));

    MockVioFrontendInterface mock_frontend;
    EXPECT_CALL(mock_frontend, detectAndDescribe(_, _, _, _)).Times(Between(12, 17));
    ...
    for (int j = 0; j < 5; ++j) {
        vio.addImuMeasurement(okvis::Time(now), imu_data.measurement.accelerometers,
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        for (int j = 0; j < 10; ++j) {
            vio.addImuMeasurement(okvis::Time(now),
